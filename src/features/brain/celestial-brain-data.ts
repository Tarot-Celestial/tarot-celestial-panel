...
export const brainNodes: BrainNode[] = [
  ...
  },
  {
    ...
    status: "attention",
    position: { x: 180, y: 90 },
    components: ["CRMClientesPanel", "ClientCapturesAdminPanel", "ReservasPanel", "CentralProfileEditor", "ReviewsPage"],
    apis: ["/api/crm/clientes/*", "/api/crm/reservas/*", "/api/central/public-profile", "/api/cliente/resenas"],
    data: ["crm_clientes", "crm_client_followups", "crm_client_capture_assignments", "central_public_profiles", "cliente_tarotista_reviews", "central-profile-photos"],
    ...
    lastChecked: "7 sep 2026 · código, producción y permisos",
    recentErrors: ["Perfiles y reseñas verificados; el resto de tablas CRM mantiene puntos de atención de políticas RLS ya inventariados."],
    children: ["Fichas", "Seguimientos", "Reservas", "Perfiles", "Reseñas"],
  },
  {
    id: "realtime",
  ...
  },
  {
    ...
    apis: ["/api/central/xp-system", "/api/admin/xp-system"],
    data: ["worker_xp_events", "worker_xp_rules", "worker_xp_level_config", "worker_xp_missions", "worker_xp_reward_claims", "worker_xp_reward_processing", "worker_coin_wallets", "worker_xp_coin_conversions"],
    ...
    importance: "Alta",
    impact: "Los eventos, canjes y recompensas usan referencias únicas; los totales se agregan en la base sin descargar el historial global.",
    sources: ["src/features/central/useCentralXpData.ts", "src/app/api/central/xp-system/route.ts", "src/app/api/admin/xp-system/route.ts"],
    lastChecked: "7 sep 2026 · código, producción e idempotencia",
    recentErrors: ["Sin errores activos: claim_id y operation_id existen, son obligatorios y mantienen índices únicos."],

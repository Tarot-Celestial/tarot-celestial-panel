...
import { BadgePlus, Coins, Contact, Save, Tag, X } from "lucide-react";
...
export default function CentralNewClientModal({ open, onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const response = await fetch("/api/crm/etiquetas/listar", { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" });
        const payload = await json(response);
        if (response.ok && payload?.ok) setTags(Array.isArray(payload.etiquetas) ? payload.etiquetas : []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      onCreated(clientId);
      onClose();
      tcToast({ title: "Clienta creada correctamente", description: `${form.nombre.trim()} ya forma parte de tu cartera.`, tone: "success", duration: 4200 });
  ...
  return (
    <div className={styles.backdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
      <section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="new-client-title">
        <header className={styles.header}><div className={styles.mark}><Contact /></div><div><span>ALTA CRM · {brand.toUpperCase()}</span><h2 id="new-client-title">Nueva clienta</h2><p>Mismo registro, saldos y etiquetas del CRM real.</p></div><button type="button" onClick={onClose} disabled={saving} aria-label="Cerrar formulario"><X /></button></header>
        <form onSubmit={submit} className={styles.form}>
          <div className={styles.scroll}>
            ...
            <fieldset><legend><BadgePlus /> Procedencia</legend><div className={styles.grid}><label className={styles.full}><span>Origen</span><input value={origin} readOnly aria-describedby="origin-help"/><small id="origin-help">Asignado automáticamente según la marca activa.</small></label></div></fieldset>
          ...
          <footer><button type="button" className={styles.cancel} onClick={onClose} disabled={saving}>Cancelar</button><button type="submit" className={styles.submit} disabled={saving}><Save size={17}/>{saving?"Creando…":"Crear clienta"}</button></footer>
        </form>
      </section>
    </div>
  );

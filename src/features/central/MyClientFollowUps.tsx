...
export default function MyClientFollowUps({ clientId, client, summary, lastPurchaseAt, onRefresh }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const json = await res.json().catch(() => null); if (!res.ok || !json?.ok) throw new Error(json?.error || "No se pudieron cargar los seguimientos.");
      setItems(Array.isArray(json.data) ? json.data : []);
    } catch (e: any) { setError(e?.message || "No se pudieron cargar los seguimientos."); }
  ...
  return (
    <section className={styles.layout}>
      ...
      {modalOpen && <div className={styles.backdrop} role="presentation"><div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="followup-title"><header><div><span>SEGUIMIENTO LEONARIS</span><h3 id="followup-title">Nuevo seguimiento</h3></div><button type="button" onClick={() => setModalOpen(false)} aria-label="Cerrar"><X size={20} /></button></header><div className={styles.formGrid}>

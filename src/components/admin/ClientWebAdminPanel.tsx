  ...
  return <section className={styles.root}>
    ...
    {selected ? <div className={styles.backdrop} onMouseDown={(e) => { if (e.target === e.currentTarget) setSelected(null); }}><div className={styles.modal}>
      ...
      <div className={styles.modalActions}>
        ...
        {selected.web_access ? <button onClick={() => { setPasswordOpen(true); setMessage(""); }}><KeyRound size={15}/> Restablecer contraseña</button> : null}
    ...
    {selected && passwordOpen ? <div className={styles.backdropTop}><div className={styles.smallModal}><div className={styles.modalHeader}><div><div className={styles.eyebrow}>ACCESO SEGURO</div><h2>Restablecer contraseña</h2></div><button className={styles.close} onClick={() => setPasswordOpen(false)}>×</button></div><p>La contraseña actual nunca se muestra ni se recupera. Solo se establecerá una nueva.</p><label>Nueva contraseña<input type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)}/></label><label>Confirmar contraseña<input type="password" autoComplete="new-password" value={confirm} onChange={(e) => setConfirm(e.target.value)}/></label>{message ? <div className={styles.message}>{message}</div> : null}<div className={styles.dialogActions}><button onClick={() => setPasswordOpen(false)}>Cancelar</button><button className={styles.primaryButton} disabled={busy || password.length < 8 || password !== confirm} onClick={() => void runAction({ action: "password", password, confirm })}>{busy ? "Cambiando…" : "Cambiar contraseña"}</button></div></div></div> : null}

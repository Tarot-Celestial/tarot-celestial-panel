...
function cardBorder(lead: LeadItem) {
  const p = String(lead.priority || "").toLowerCase();
  if (p === "critical") return "1px solid rgba(255,98,98,.55)";
  if (p === "high") return "1px solid rgba(255,196,98,.45)";
  return "1px solid rgba(255,255,255,.10)";
}

export default function CaptacionPanel({ mode }: { mode?: "admin" | "central" | string }) {
  ...
  useEffect(() => {
    const i = window.setInterval(() => load(false), 15000);
    return () => window.clearInterval(i);
  }, [view]);

  useEffect(() => {
    const channel = sb
      .channel(`captacion-live-${mode || "panel"}`)
  ...
  const stats = useMemo(() => {
    ...
    const due = items.filter((x) => !!x.due_now && !["contactado", "no_interesado", "numero_invalido", "perdido"].includes(String(x.estado || ""))).length;
        ...
        <div className="tc-row" style={{ justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div>
            ...
            <div className="tc-sub" style={{ marginTop: 6 }}>
              Leads nuevos y reintentos para llamar en el momento correcto.
            </div>
          </div>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {items.map((lead) => (
              <div
                ...
                style={{
                  borderRadius: 18,
                  border: cardBorder(lead),
                  background: lead.due_now ? "rgba(181,156,255,0.08)" : "rgba(255,255,255,0.03)",
                  padding: 14,
                }}
              >
                <div className="tc-row" style={{ justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
                  ...
                  <div className="tc-row" style={{ gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    {!!lead.overdue_minutes ? <div className="tc-chip">+{lead.overdue_minutes} min</div> : null}
                  </div>
                </div>

...
export async function POST(req: Request) {
  try {
    ...
    const incident_id = String(body.incident_id || "");
    const status = String(body.status || "unjustified"); // justified | unjustified
    const note = String(body.note || "");

    if (!incident_id) return NextResponse.json({ ok: false, error: "MISSING_ID" }, { status: 400 });

...
export async function POST(req: Request) {
  try {
    ...
    let tiempo: number, importe: number, blocks;
    try {
      ...
      if (!updates || Object.keys(updates).some(key => !["tiempo","importe","code_blocks"].includes(key))) throw new Error("Solo se pueden corregir tiempo, importe y códigos.");
    ...
    const { data, error } = await admin.rpc("tc_rendimiento_update", {
      p_worker: worker.id, p_id: body.id, p_revision: body.revision,
      p_tiempo: tiempo, p_importe: importe, p_blocks: blocks,
    });
    if (error) {

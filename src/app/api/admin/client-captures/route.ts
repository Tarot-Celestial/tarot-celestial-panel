...
export async function GET(req:Request){
 try{
  ...
  // Solo se pueden asignar carteras a perfiles conectados con una cuenta real.
  // Así evitamos filas antiguas/duplicadas que se llaman igual que María, Yami, etc.
  const activeWorkers=(workers||[]).filter((x:any)=>x.is_active!==false&&Boolean(x.user_id));
...
export async function POST(req:Request){
 try{const me=await requireAdmin(req);if(!me)return NextResponse.json({ok:false,error:"FORBIDDEN"},{status:403});const body=await req.json().catch(()=>({}));const clientId=String(body.client_id||"");const workerId=body.responsible_worker_id?String(body.responsible_worker_id):null;if(!clientId)return NextResponse.json({ok:false,error:"CLIENT_REQUIRED"},{status:400});const db=adminClient();if(workerId){const target=await db.from("workers").select("id,user_id,role,is_active").eq("id",workerId).maybeSingle();if(target.error)throw target.error;if(!target.data||target.data.role!=="central"||target.data.is_active===false||!target.data.user_id)return NextResponse.json({ok:false,error:"La responsable seleccionada no está conectada a una cuenta central activa."},{status:400});}const {error}=await db.rpc("reassign_client_capture_responsible",{p_client_id:clientId,p_new_worker_id:workerId,p_actor_worker_id:me.id,p_reason:String(body.reason||"")});if(error)throw error;const persisted=await db.from("crm_client_capture_assignments").select("client_id,responsible_worker_id,captured_by_worker_id,business,updated_at,status").eq("client_id",clientId).maybeSingle();if(persisted.error)throw persisted.error;const actual=persisted.data?.responsible_worker_id?String(persisted.data.responsible_worker_id):null;if(actual!==workerId)throw new Error("La asignación no quedó persistida");return NextResponse.json({ok:true,assignment:persisted.data});}

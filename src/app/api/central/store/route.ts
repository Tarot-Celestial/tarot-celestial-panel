...
export async function GET(req:Request){
 try{
  const {me,admin}=await storeAccess(req); if(!me)return NextResponse.json({ok:false,error:"NO_AUTH"},{status:401});
  ...
  const [walletR,rewardsR,claimsR,eventsR,levelConfig]=await Promise.all([
   ...
   admin.from("worker_store_rewards").select("id,name,description,category,coin_cost,icon_key,image_url,featured,active,display_order,stock,required_level,updated_at").eq("active",true).is("archived_at",null).order("display_order").order("created_at"),
   admin.from("worker_store_claims").select("id,reward_id,reward_name,category,coin_cost,status,status_note,reward_image_url,created_at,updated_at").eq("worker_id",String(me.id)).order("created_at",{ascending:false}),
 ...
 }catch(e:any){return NextResponse.json({ok:false,error:e?.message||"STORE_NOT_INSTALLED"},{status:e.status||500});}
}
...
export async function POST(req:Request){
 try{
  ...
  if(!Number.isSafeInteger(expectedCost)||expectedCost<=0)return NextResponse.json({ok:false,error:"INVALID_PRICE"},{status:400});
  const result=await admin.rpc("claim_worker_store_reward",{p_worker_id:String(me.id),p_reward_id:rewardId,p_operation_id:operationId,p_expected_cost:expectedCost});
  if(result.error){const message=String(result.error.message||"");const known=["PRICE_CHANGED","INSUFFICIENT_COINS","REWARD_UNAVAILABLE","REWARD_OUT_OF_STOCK","LEVEL_REQUIRED","OPERATION_ID_CONFLICT"].find(x=>message.includes(x));return NextResponse.json({ok:false,error:known||"CLAIM_FAILED"},{status:409});}
  return NextResponse.json({ok:true,claim:result.data});
 }catch(e:any){return NextResponse.json({ok:false,error:e?.message||"CLAIM_FAILED"},{status:e.status||500});}
}

...
export async function GET(req: Request) {
  try {
    ...
    // ✅ solo central (y admin si quieres)
    ...
    if (!me || (role !== "central" && role !== "admin")) {
      return NextResponse.json({ ok: false, error: "FORBIDDEN" }, { status: 403 });
    }

    // ✅ reutilizamos el endpoint admin ya existente (mismo dominio)
    // En vez de HTTP interno, replicamos la lógica llamando a la misma tabla:
    // Para evitar duplicar TODO el cálculo aquí, hacemos lo más robusto:
    // -> consultamos directamente shift_schedules y devolvemos tarotistas en turno.
    // (Si prefieres 100% idéntico al admin, dime y lo clonamos literal.)

    const TZ = "Europe/Madrid";

    // Helpers mínimos (copiados y reducidos)
    ...
    expected.sort((a: any, b: any) =>
      String(a.worker?.display_name || "").localeCompare(String(b.worker?.display_name || ""))
    );
